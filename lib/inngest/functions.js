...
export const sendBudgetAlertForTransaction = inngest.createFunction(
  {
    id: "send-budget-alert-for-transaction",
    name: "Send Budget Alert For Transaction",
    triggers: [{ event: "budget.alert.check" }],
  },
  async ({ event, step }) => {
    const { userId, accountId } = event.data || {};
    if (!userId || !accountId) return { skipped: "missing-event-data" };

    return await step.run("check-and-send-budget-alert", async () => {
      ...
      if (percentageUsed < 80 || alreadySentThisMonth) {
        return { skipped: "below-threshold-or-already-sent", percentageUsed };
      }
      ...
      return { sent: emailResult.success, percentageUsed };
    });
  }
);
...
export const processRecurringTransaction = inngest.createFunction(
  {
    ...
    triggers: [
      { event: "transaction.recurring.process" }, // 👈 belongs here
    ],
  },
  async ({ event, step }) => {
    ...
    await step.run("process-transaction", async () => {
      ...
      await processDueRecurringTransactions(transaction.userId);
    });
  }
);
